        ...
        <div class="section" v-for="(section, index) in content.data" :key="index">
          ...
          <ul class="section-list">
            <li class="section-item" v-for="(item, i) in section.itemList" :key="i">
              <a :href="item.linUrl" class="section-link">
...
import { CATEGORY_CONTENT_KEY, CATEGORY_CONTENT_UPDATE_THE_INTERVSL } from './config';
  ...
  methods: {
    getContent(id) {
      ...
      if (contents && contents[id]) {
        ...
        if (curTime - updateTime <= CATEGORY_CONTENT_UPDATE_THE_INTERVSL) { // localStorage
